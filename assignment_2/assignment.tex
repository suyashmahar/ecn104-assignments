...
\begin{table}[!ht]
  \centering
  \caption{\centering{Common use cases of wires and reg for synthesizable Verilog code. l-value refers to a net or variable to which an assignment is made while an r-value refers to a net or variable that can be used standalone or in combination with other net or variable to assign a value. Wires are declared as: \inlinev{wire w0, w1} and Reg are: \inlinev{reg r0, r1, r2};}}
  \label{table:reg-wire-use}
  \renewcommand{\arraystretch}{1.1}
  \begin{tabularx}{\textwidth}{|X|X|c|c|X|}
    \hline
    \rowcolor{greatblue}
    \multicolumn{2}{|l|}{\color{white}Use case}  & {\noindent\color{white}Reg} & {\noindent\color{white}wire} &  {\noindent\color{white}Example}\\
    \hline

    \multicolumn{2}{|l|}{\textbf{Multi-bit declaration}}  & \markupg{\checkmark} & \markupg{\checkmark} & \footnotesize\inlinev{reg [32-1:0] name;} \inlinev{wire [32-1:0] name;} \\
    \multicolumn{2}{|l|}{\footnotesize(Single dimensional)} &  &  &   \\
    \hline

    \multicolumn{2}{|l|}{\textbf{Multi-bit declaration}}  & \markupg{\checkmark} & \markupg{\checkmark} & \footnotesize\inlinev{reg [DIM_1-1:0] name [DIM_2-1:0][DIM_3-1:0];} \\
    \multicolumn{2}{|l|}{\footnotesize(Multi-dimensional)} &  &  & \footnotesize\inlinev{// Usage: name[addr_2][addr_3][addr_1]}\\
    \hline

    \multicolumn{5}{|l|}{\textbf{Continuous Assignment}}\\
    \hline
    \multicolumn{2}{|r|}{\footnotesize{l-value}} & \markupr{$\boldsymbol{\times}$} & \markupg{\checkmark} & \footnotesize\inlinev{assign w0 = r0 & r2;}\\
    \multicolumn{2}{|r|}{\footnotesize{r-value}} & \markupg{\checkmark}            & \markupg{\checkmark} & \footnotesize\inlinev{assign w0 = r0 & w1; // r-value can have either wire or register}\\
    \hline
    
    \multicolumn{5}{|l|}{\textbf{Initial Block}}\\
    \hline
    \multicolumn{2}{|r|}{\footnotesize{l-value}} & \markupg{\checkmark}            & \markupr{$\boldsymbol{\times}$} & \footnotesize\inlinev{r0 = 1'b0;}\\
    \multicolumn{2}{|r|}{\footnotesize{r-value}} & \markupg{\checkmark}            & \dangersign                     & \footnotesize\inlinev{r1 = r0; // NOTE: r0 should be initialized first}\\
    \multicolumn{2}{|r|}{}                       &                                 &                                 & \footnotesize\inlinev{r2 = w0; // WARNING: Should be avoided} {\scriptsize\color{gray} (Please refer to Pitfall \ref{pitfall:init-with-wire})}\\ 
    \hline
    
    \multicolumn{5}{|l|}{\textbf{Procedural Block}}\\
    \hline
    \multicolumn{2}{|r|}{\footnotesize{l-value}} & \markupg{\checkmark}            & \markupr{$\boldsymbol{\times}$} & \footnotesize\inlinev{r0 = 1'b0;}\\
    \multicolumn{2}{|r|}{\footnotesize{r-value}} & \markupg{\checkmark}            & \markupg{\checkmark}            & \footnotesize\inlinev{r1 = r0;}  \\
    \multicolumn{2}{|r|}{}                       &                                 &                                 & \footnotesize\inlinev{r2 = w0; // This statement is perfectly valid in proc. blocks}\\
    \hline
    
    \multicolumn{5}{|l|}{\textbf{Module's connections}}\\
    \multicolumn{5}{|l|}{\footnotesize(from outside of the module)}\\
    \hline 
    \multicolumn{2}{|r|}{\footnotesize{l-value}} & \markupg{\checkmark}            & \markupg{\checkmark}            & \multicolumn{1}{l|}{\multirow{2}*{\footnotesize{Refer to the material ``The art of writing testbenches''}}}\\
    \multicolumn{2}{|r|}{\footnotesize{r-value}} & \markupr{$\boldsymbol{\times}$} & \markupg{\checkmark}            & \multicolumn{1}{l|}{}\\ 
    \hline
    
    \multicolumn{5}{|l|}{\textbf{Module's IO Declaration}}\\
    \multicolumn{5}{|l|}{\footnotesize(from inside the module)}\\
    \hline
    \multicolumn{2}{|r|}{\footnotesize{l-value}} & \markupg{\checkmark}            & \markupg{\checkmark}            & \multicolumn{1}{l|}{\multirow{2}*{\footnotesize{Refer to the material ``The art of writing testbenches''}}}\\
    \multicolumn{2}{|r|}{\footnotesize{r-value}} & \markupr{$\boldsymbol{\times}$} & \markupg{\checkmark}            & \multicolumn{1}{l|}{}\\
    \hline    
 ...
 \insertProblem {35\%} {
   ...
   \begin{table}[!h]
     ...
     \begin{tabularx}{0.8\textwidth}{|X|X|X|}
       ...
       Testbench Name  & equation\_lut\_tb &  -  \\
       \hline
     \end{tabularx}
   \end{table}

   \hint{hint:problem-4}{ An example LUT for a not gate will have a
     two location array of reg (\inlinev{reg [2-1:0] mem}) and will
     have content \inlinev{\{1,0\}}, thus when the input address to
     the LUT is 0 the output will be 1 while when the input is 1 the
     output will be 0. This way the LUT will behave as a NOT gate. You
     have to implement other gates similarly.} }
